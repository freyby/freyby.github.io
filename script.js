...
function renderUserInfo(userData) {
  ...
  userCreatedAt.textContent =
    "Account created at: " + new Date(userData.createdAt).toLocaleString();
  ...
  auditsRatioPara.textContent =
    "Audits Ratio: " + auditsData.auditsRatio.toFixed(2);
  ...
  xpProgressionElement.textContent =
    "XP Progression: " + xpProgression / 1000 + "kB";
...
function renderPieChart(upAmount, downAmount) {
  ...
  PercentageText.textContent =
    receivedPercentageString + " / " + donePercentageString;
...
function renderXPProgressionGraph(transactions) {
  ...
  const xpTransactions = transactions.filter(
    (transaction) =>
      transaction.type === "xp" &&
      !transaction.path.includes("piscine") &&
  ...
  const timestamps = xpTransactions.map(
  ...
  const maxXP = d3.max(cumulativeXP); // Assuming cumulativeXP is defined and calculated
  yScale.domain([0, maxXP]);

  const yAxis = d3.axisLeft(yScale).tickFormat((d) => {
    return d >= 1000000 ? `${d / 1000000} MB` : `${d / 1000} kB`;
  });
